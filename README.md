...
### File header comment
In order to add a header comment at the top of the generated files, pass the `gen-header-comment` option with the `as_opt` parameter:

```sh
protoc --plugin=protoc-gen-as=./node_modules/.bin/as-proto-gen --as_opt=gen-header-comment --as_out=. ./file.proto
```

This will generate a header comment similar to the one below:
```typescript
