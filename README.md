...
export function decodeStarRepoMessage(a: Uint8Array): StarRepoMessage {
  return Protobuf.decode<StarRepoMessage>(a, StarRepoMessage.decode);
}
...
The as-proto-gen plugin support an option to also generate AssemblyScript file for dependencies of the proto file passed as argument to protoc.
Add `--as_opt=gen-dependencies` to use it.

```sh
protoc --plugin=protoc-gen-as=./node_modules/.bin/as-proto-gen --as_opt=gen-dependencies --as_out=. ./file.proto
```
